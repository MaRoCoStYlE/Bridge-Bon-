...
// 1) Servez le mapping statique depuis /mapping.json (collez votre b_mapping.json ici)
...
// 4) Création du cart via Storefront
async function createCheckout(shopKey, payload){
  ...
  const lines = (payload.lines||[]).map(l => ({
    quantity: l.quantity,
    merchandiseId: l.id,                // ID variant GID côté boutique cible
...
// 5) Endpoint principal: reçoit le panier de A, route vers B (ou C), redirige 302
